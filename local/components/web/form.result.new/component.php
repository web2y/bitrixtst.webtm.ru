...
{
	$APPLICATION->SetAdditionalCSS($this->GetPath()."/style.css");
	$GLOBALS['strError'] = '';
	...
	if (strlen($arResult["ERROR"]) <= 0)
	{
		...
		*/
		...
		if (!empty($_REQUEST["formresult"]) && $_REQUEST['WEB_FORM_ID'] == $arParams['WEB_FORM_ID'])
		{
			...
			$arMyAnswer = CFormResult::GetDataByID(
		    $_REQUEST['RESULT_ID'], 
			array(),
			$arMyResult, 
			$arMyAnswer2);
			...
			if (!$bCache || $bCache && !$bVarsFromCache)
			{
				...
				if(isset($_REQUEST['RESULT_ID']) && !empty($_REQUEST['RESULT_ID'])):
					...
					$arMyAnswer = CFormResult::GetDataByID(
					    ...
					    $arMyAnswer2);
					$arValueInput = array();
					foreach ($arMyAnswer as $key => $arItem) {
					    # code...
					    foreach ($arItem as $key => $arValue) {
					        if( $arValue["FIELD_TYPE"] == 'radio' ) {
					            $arValueInput["form_radio_".$arValue["SID"]] = $arValue["ANSWER_ID"];
					        }
					        if( $arValue["FIELD_TYPE"] == 'text' ) {
					            $arValueInput["form_text_".$arValue["ANSWER_ID"]] = $arValue["USER_TEXT"];
					        }
					        if( $arValue["FIELD_TYPE"] == 'dropdown' ) {
					            $arValueInput["form_dropdown_".$arValue["SID"]] = $arValue["ANSWER_ID"];
					        }
					        if( $arValue["FIELD_TYPE"] == 'email' ) {
					            $arValueInput["form_email_".$arValue["ANSWER_ID"]] = $arValue["USER_TEXT"];
					        }
					        if( $arValue["FIELD_TYPE"] == 'textarea' ) {
					            $arValueInput["form_textarea_".$arValue["ANSWER_ID"]] = $arValue["USER_TEXT"];
					        }
					    }
					}

					$arResult["arrVALUES"] = $arValueInput;
				endif;
